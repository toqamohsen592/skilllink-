  ...
  <div class="step-page">
    ...
    <div class="step-container">
      ...
      <div class="form-card">
        ...
        <div class="form-row"><label>Enter country</label><select v-model="country"><option value="">Select country</option><option value="+20">Egypt (+20)</option><option value="+966">KSA (+966)</option></select></div>
        <div class="form-row"><label>Enter your phone number</label><input v-model="phone" placeholder="phone number"></div>
...
<script>
import { getSignupDraft, saveSignupDraft, clearSignupDraft } from '../utils/signupStorage'
import { saveLocalStudentProfile } from '../utils/profileStorage'
import { mapSignupDraftToApi, mapSignupDraftToLocalProfile, getStoredUser } from '../utils/profileMapper'
...
export default {
  ...
  methods: {
    async finishSignup() {
      ...
      this.saving = true
      saveSignupDraft({ country: this.country, phone: this.phone })
      const draft = getSignupDraft() || {}
      const localProfile = mapSignupDraftToLocalProfile(draft)
      saveLocalStudentProfile(localProfile)

      const token = localStorage.getItem('token')
      if (token) {
        try {
          const payload = mapSignupDraftToApi(draft)
          ...
          console.error('Failed to save signup profile to API', e)
        }
      }
