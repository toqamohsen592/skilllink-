...
import { clearSignupDraft, getSignupDraft } from '../utils/signupStorage'
import { mapClientProfileToApi } from '../utils/profileMapper'
...
export default {
  ...
  methods: {
    ...
    async completeSignup() {
      ...
      if (localStorage.getItem('token') && draft?.profileData) {
        try {
          await clientAPI.updateClientProfile(mapClientProfileToApi(draft.profileData))
        } catch (e) {
          console.error('Failed to save client profile during signup', e)
        }
      }
      clearSignupDraft()
