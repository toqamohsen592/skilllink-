...
import { mapStudentProfileFromApi, getStoredUser } from '../utils/profileMapper'
      ...
      try {
        ...
        if (token) {
          try {
            ...
            this.profile = mapStudentProfileFromApi(data, getStoredUser())
            return
          } catch (e) {
        ...
        const mapped = localStudentToProfileView(local)
        if (mapped) {
