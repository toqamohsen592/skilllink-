<template>
  <div class="client-profile-page">
    ...
    <div class="profile-container">
      <div class="profile-sidebar">
        <div class="sidebar-profile-pic">
          <img :src="profile.profile_picture || '/src/assets/pexels danx.jpg'" :alt="profile.full_name" class="sidebar-profile-img">
        </div>
      ...
      <div class="profile-main">
        ...
        <template v-else>
          <div class="profile-header">
            <h1 class="profile-name">{{ profile.full_name || 'Client' }}</h1>
            <p class="profile-title">{{ profile.title || profile.account_type || 'Business Owner' }}</p>
          ...
          <div class="profile-section">
            ...
            <div class="company-info-grid">
              <div class="company-left">
                <div class="info-item"><h4>Company Name</h4><p>{{ profile.company_name || 'Not provided' }}</p></div>
                <div class="info-item"><h4>Industry</h4><p>{{ profile.industry || 'Not provided' }}</p></div>
                <div class="info-item"><h4>Website</h4><p>{{ profile.website || 'Not provided' }}</p></div>
                <div class="info-item"><h4>Location</h4><p>{{ profile.location || 'Not provided' }}</p></div>
              </div>
              <div class="company-right">
                <h4>Description</h4>
                <p>{{ profile.company_description || profile.bio || 'No company description added yet.' }}</p>
              </div>
            </div>
          </div>
          <div class="profile-section">
    ...
    <!-- Robot Icon -->
    <div class="robot-wrapper" @click="toggleChatbot"><div class="robot-circle"><img src="/src/assets/Chat bot.png" alt="Robot" class="robot-img"></div></div>
...
import { getLocalClientProfile, localClientToProfileView } from '../utils/profileStorage'
import { mapClientProfileFromApi } from '../utils/profileMapper'
  ...
  methods: {
    mapProfile(data) {
      ...
      return {
        ...
        industry: p.industry || '',
        website: p.website || '',
        company_description: p.company_description || p.companyDescription || '',
        account_type: p.account_type || p.accountType || 'Business Owner',
      ...
      try {
        ...
        if (token) {
          try {
            ...
            this.profile = mapClientProfileFromApi(data)
            return
          } catch (e) {
        ...
        const mapped = localClientToProfileView(local)
        if (mapped) {
          this.profile = { ...this.profile, ...mapped }
        }
      } finally {
        this.loading = false
      }
