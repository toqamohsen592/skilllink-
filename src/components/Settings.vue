      ...
      <div class="settings-content">
        ...
        <div v-if="activeTab === 'profile'" class="settings-section">
          ...
          <div class="profile-photo-section">
            <img :src="profilePhoto || '/src/assets/Headshot profile.jpg'" alt="Profile" class="profile-photo">
...
import {
  applyLocalStudentProfile,
  getLocalStudentProfile,
  saveLocalStudentProfile
} from '../utils/profileStorage'
import {
  mapStudentProfileToApi,
  mapStudentSettingsFromApi,
  getStoredUser
...
export default {
  ...
  data() {
    return {
      ...
      accountData: { email: '', username: '', accountType: 'Student' },
      profileData: { fullName: '', bio: '', university: '', major: '', skills: [] },
  ...
  methods: {
    ...
    mapFromApi(profileData) {
      const mapped = mapStudentSettingsFromApi(profileData, getStoredUser())
    ...
    async loadProfile() {
      ...
      if (token) {
        try {
          ...
          this.mapFromApi(p)
          saveLocalStudentProfile(this.getLocalSnapshot())
          return
        } catch (e) {
          console.error('Failed to load profile settings from API', e)
        }
      }
      applyLocalStudentProfile(this)
    },
    ...
    async saveProfile() {
      const payload = mapStudentProfileToApi(this.profileData)
      ...
      try {
        ...
        if (!token) {
          saveLocalStudentProfile(snapshot)
